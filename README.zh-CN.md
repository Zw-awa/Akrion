...
桌面应用面向真机调试：连接串口后即可实时查看目标值、实际值和控制量，暂停、缩放、平移、查看游标数据，并在采集结束后回放同一次运行。命令行适合自动化录制、校验和导出，GUI 适合现场观察。

CLI 包含：

- `doctor`、`ports`、`init`、`record`、`demo`
- `components`（查看当前可用的输入、噪声和算法组件）
...
开发时使用 `run` 启动。要生成包含 Qt 运行库、可直接启动的 Windows 应用目录：
...
统一入口按“命令行参数、环境变量、`.env`、自动发现”的顺序定位 Qt，并固定使用该安装中的 CMake、MinGW 和 `MinGW Makefiles`。启动时会隔离 PATH 中其他软件附带的 Qt 运行库。需要本地配置时，将 `.env.example` 复制为 `.env` 并填写 `QT_ROOT`、`QT_VERSION`；`.env` 不会被 Git 提交。检测到 Codex 时自动启用 agent-safe CMake 分支，普通终端仍使用标准 `qt_add_qml_module`。
...
组件库目前提供稳定的 C ABI。组件接收带时间和通道 ID 的结构化输入，并把输出写入 Akrion 提供的缓冲区。C、C++ 和 Rust 组件都可以使用同一套接口；Rust 示例位于 [`sdk/rust/akrion-component`](./sdk/rust/akrion-component)。
